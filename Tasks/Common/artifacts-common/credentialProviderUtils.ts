...
export async function installCredProviderToUserProfile(overwrite: boolean) {
    ...
    const netCoreSource = path.join(taskPluginsPir, 'netcore');
    const netCoreDest = path.join(userPluginsDir, 'netcore');
    ...
    if (isWin) {
        const netFxSource = path.join(taskPluginsPir, 'netfx');
        const netFxDest = path.join(userPluginsDir, 'netfx');
